...
class CommandPolicyEngine:
    ...
    def classify(self, command: str | Sequence[str]) -> PolicyResult:
        ...
        if executable == "uv" and any(token in {"add", "remove", "sync", "lock", "pip"} for token in lowered[1:]):
            return self._approval("dependency environment changes require approval", "approval.package")
        ...
        if executable == "find" and any(token in {"-exec", "-execdir", "-delete"} for token in lowered):
            return self._deny("find execution or deletion is denied", "deny.find-mutation")
        if executable == "rg" and any(token == "--pre" or token.startswith("--pre=") for token in lowered):
        ...
        if executable == "node" and len(lowered) >= 2 and lowered[1] in {"--check", "--version", "-v"}:
            return self._allow("bounded Node validation command", "allow.node")
        if executable == "npx" and len(lowered) >= 2 and lowered[1] in {
            "vitest",
            "tsc",
            "eslint",
            "prettier",
        ...
        return self._approval("unrecognized executable requires operator review", "approval.unknown")
    ...
    def _git(self, tokens: tuple[str, ...]) -> PolicyResult:
        ...
        if subcommand in {"push", "merge", "tag", "rebase", "filter-branch", "filter-repo"}:
            return self._deny("Git publication or history rewriting is disabled", "deny.git-publish")
        if subcommand == "reset" and "--hard" in tokens:
            return self._deny("destructive Git reset is denied", "deny.git-destructive")
        if subcommand == "clean" and any(token.startswith("-") and "f" in token for token in tokens[2:]):
        ...
        if subcommand in {"switch", "checkout", "worktree", "branch", "add", "commit"}:
            return self._approval("local Git mutation requires explicit configuration", "approval.git-local")
    ...
    def _node_package(self, tokens: tuple[str, ...]) -> PolicyResult:
        ...
        if tokens[1] == "test" or tokens[1:3] in {("run", "test"), ("run", "lint"), ("run", "typecheck")}:
